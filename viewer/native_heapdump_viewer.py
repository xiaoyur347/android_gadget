...
for lib in addrs_by_lib:
    ...
    if os.path.isfile(sofile):
        file_offset = 0
        result = subprocess.check_output(["arm-linux-androideabi-objdump", "-w", "-j", ".text", "-h", sofile])
        ...
        p = subprocess.Popen(["arm-linux-androideabi-addr2line", "-C", "-j", ".text", "-e", sofile, "-f"], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
